...
__host__ __device__  float boxIntersectionTest(glm::vec3 boxMin, glm::vec3 boxMax, staticGeom box, ray r, glm::vec3& intersectionPoint, glm::vec3& normal){
	...
	ray rt;
	rt.origin = ro;rt.direction = rd;
	float Tnear = - INF;float Tfar = INF;
	glm::vec3 Bl = boxMin;
	glm::vec3 Bh = boxMax;
	float T1,T2;T1=T2=0;
	double t; //the return value
	for(int i = 0;i<3;i++)//i = 0 for x axis, i = 1 for y axis, i = 2 for z axis;
	{
		if(rt.direction[i]==0)
		{
			if(rt.origin[i]>Bh[i]||rt.origin[i]<Bl[i]) {t = -1;break;}
		}
		else 
		{
			T1 = (Bl[i] - rt.origin[i])/rt.direction[i];
			T2 = (Bh[i] - rt.origin[i])/rt.direction[i];
...
__host__ __device__ glm::vec3 getRandomPointOnSphere(staticGeom sphere, float randomSeed){
  ...
  return randPoint;*/
	...
	glm::vec3 dire = glm::vec3(u(rng), u(rng), u(rng));//to check if this is the right way to use random func
	dire = glm::normalize(dire);
	float radius = getRadiuses(sphere).x;//all three dimention length would work
	ray r;
	r.origin = multiplyMV(sphere.transform, glm::vec4(0,0,0,1));
	r.direction = dire;
	glm::vec3 point = getPointOnRay(r,radius);
	return point;
}
